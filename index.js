...
async function run() {
  try {
    const categoryCollection = client.db('bigBajar').collection('products-category');
    const productsCollection = client.db('bigBajar').collection('products');
    const bookedItemCollection = client.db('bigBajar').collection('booked-items');
    const userCollection = client.db('bigBajar').collection('users');
    const advertiseCollection = client.db('bigBajar').collection('advertiseitem');
    const wishlistCollection = client.db('bigBajar').collection('wishlist');
    const paymentsCollection = client.db('bigBajar').collection('payments');
    //  post booked item
...
app.get('/', async (req, res) => {
  res.send('bigbajar Hut Server Is running')
})

app.listen(port, () => {
  console.log('bigbajar Hut server is running')
})